...
// 🔴 THIS PAGE WAS TOKENISED AND STILL PAINTED NOTHING, WHICH IS THE WHOLE POINT OF THE THEME
// GUARD. It had no theme sheet, so every var() below resolved to nothing: `color: var(--tx)` fell
// back to inherit, `background: var(--panel)` to transparent, and `border: 1px solid var(--line)`
// to currentColor, which draws a full strength INK hairline where a pale one was meant. Being on
// tokens is not the same as being handed a palette to read them from.
//
// ⚠️ AND TWO OF THE NAMES WERE NOT REAL. `--muted` and `--line` are declared nowhere in
// lib/tokens.ts. The real ones are `--tx-mut` and `--bd`. A var() with no fallback fails silently,
// so a typo in a custom property name is invisible until somebody opens the page.
const INK = 'var(--tx)';
...
function Shell({ children }: { children: React.ReactNode }) {
  return (
    <>
      {/* APP_THEME_CSS, not THEME_CSS: no shell here, so nothing would ever set data-theme. */}
      ...
      {cats.length > 0 ? (
        <>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: 14, fontSize: 15 }}>
            <tbody>
              {cats.map((c) => (
                <tr key={c.category} style={{ borderBottom: `1px solid ${LINE}` }}>
