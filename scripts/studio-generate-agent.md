...
1. Fetch the queue:
   ...
   It returns `{ pending, assets: [{ id, title, trade, format, caption, scene, platforms, storyboard }] }`.
   These are the approved assets that still need a video. If `pending` is 0, stop.

2. For each asset, generate the media with the Higgsfield connector, guided by its storyboard:

   - **video**: one short vertical clip, 12 to 25 seconds. Faceless, a real workplace, natural light,
     handheld. Follow the frames in order. Each frame: `visual` is the shot, `caption` is the words to
     burn on screen, `vo` is the voiceover line, `seconds` is how long it holds.
   - **carousel**: one image per frame. Flat illustration, one colour world, the `caption` as the big
     on screen line. It must read on mute.
   - **tip**: generate the tip card images from the frames.

   Keep it on brand: honest, blunt, British, no faces, and nothing that claims we file tax or that the
   character is a real customer. Use `scene` for the mood.
...
3. Attach the hosted media URL Higgsfield returns:
   ...
   `{"updated":true}` means done. `{"updated":false}` means it was already generated, so skip it.

4. When the queue is empty, stop and report a one line summary of what you generated.

## Hard rules

- Only generate for assets returned by `pending-generation`. Those are the approved ones.
