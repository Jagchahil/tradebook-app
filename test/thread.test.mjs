...
`);
  ...
  const post = async ({ q, image, library, rows = [] }) => {
    ...
    if (image) fd.append('receipt', new Blob([new Uint8Array(JPEG_BYTES)], { type: 'image/jpeg' }), 'r.jpg');
    if (library) fd.append('receipt_library', new Blob([new Uint8Array(JPEG_BYTES)], { type: 'image/jpeg' }), 'r.jpg');
