...
export const GATED_ROUTES: GatedRoute[] = [
  ...
  { route: 'app/api/elections', rule: 'entitled', why: 'Claiming use of home, or the flat trading allowance. Us finding him money is the product. ⚠️ Only ELECTING is gated. Removing deliberately is not, and it now arrives two ways: a DELETE from the phone app, and intent=remove on the POST from the web, which ships no client script and so cannot send a verb. Both sit above the gate check on purpose: he may always UNDO a claim on his own record, and refusing that would leave an election standing that he has asked us to drop.' },
  { route: 'app/api/diary/photo/view', rule: 'always', why: 'His own photograph, as pixels, for the job screen. 🔴 NOT GATED, and it is the diary read\'s argument exactly: looking at his own record is never the work, and a lapsed card must never hide his evidence from him. It writes nothing, it reads one row filtered on his session AND the row id, and it refuses any storage path outside his own folder. Keeping a NEW photograph is the work and is gated, on app/api/diary/photo.' },
  { route: 'app/api/diary/photo', rule: 'entitled', why: 'A photograph kept against a job. Keeping one is new work and it costs storage, so it sits with adding a job. ⚠️ TAKING ONE BACK DOWN IS NOT HERE: photo-remove lives on app/api/diary and is ungated, because undoing his own act must never cost £12.99, the elections DELETE shape. And a photograph moves no tax figure, so it has no approval gate either: everything that moves a number still asks him.' },
  ...
  { route: 'app/api/money/upload', rule: 'entitled', why: 'The one door for uploads: receipts and statements together, sorted by the door, walked by the same two pipelines the single-file routes call. The same work, so the same rule, and his existing rows stay readable everywhere.' },
  ...
  { route: 'app/api/you/details', rule: 'always', why: 'His own name, business name, business address and trade. Facts about himself, not work we do for him, and until this route existed reconcileSignupToUser was the only writer of any of them in the repo, so a step tapped past at /start was permanent. The address is a field GOV.UK asks every invoice to carry, so gating it would put a legal requirement behind a paywall.' },
  ...
  { route: 'app/api/testimonial', rule: 'always', why: 'His own review of us, written and withdrawn from inside his account. Both directions are always, and for different reasons. Writing one is HIM DOING US A FAVOUR: there is no work of ours behind it and no cost to us, and charging a man for the privilege of praising us is the wrong shape of business. Taking it down is stronger than that: it is withdrawal of consent under Article 7(3), which has to be as easy as giving it, so a lapsed subscription must never be able to hold a man\'s name on a marketing page. Publishing is the only part nobody can do from here, and that is a team decision at /team rather than a paid one.' },
