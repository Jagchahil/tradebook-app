...
// 🔴 THIS PAGE IS 39 ROUTES, NOT ONE, AND IT WAS UNTHEMED UNTIL 5 AUGUST.
//
// The theme guard in test/tokens.test.mjs said zero public pages were unthemed while this file sat
// on hardcoded light hex, because the guard's page list dropped every path containing a bracket and
// called them "dynamic segments whose fixtures are not routes". dynamicParams = false and
// generateStaticParams() twelve lines up: these are 39 prerendered SEO landing pages, in the
// sitemap, and for a lot of arrivals this file IS the website. The exclusion is now gone.
//
// ⚠️ AND THE GUARD PASSED IT ON ONE LINE: import { Ic } from '../../_shared/site'. An icon. The
// test matched the shell module's import PATH rather than the shell being rendered, which is the
// third time in two days that a check has been satisfied by a mention instead of an effect.
//
// ⚠️ AND THE CTA BAND BELOW IS THE Wizard.tsx TRAP AGAIN. It was background: INK with white text,
// a deliberately DARK band on a light page. INK is var(--tx), which goes near WHITE in dark, so the
// obvious tokenisation turns it into a white band with white text on it. BAND is dark in BOTH
// appearances, which is what a deliberately dark band actually means.
const INK = 'var(--tx)';
...
export default async function TradePage({ params }: { params: Promise<{ trade: string }> }) {
  ...
  return (
    <main style={{ backgroundColor: PAPER, color: INK, fontFamily: FONT, overflowX: 'hidden' }}>
      ...
      {/* APP_THEME_CSS, not THEME_CSS. This page has no shell, so nothing sets data-theme on it and
          THEME_CSS's dark half could never match. APP_THEME_CSS hangs off prefers-color-scheme. */}
      ...
      <section style={{ maxWidth: 1180, margin: '0 auto', padding: '26px 24px 8px' }}>
        <div style={{ maxWidth: 720 }}>
          ...
          <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
            <Link href="/start" style={{ backgroundColor: RIVER, color: ON_RIVER, fontSize: 16, fontWeight: 600, padding: '15px 30px', borderRadius: 12 }}>Start free trial</Link>
      ...
      <section style={{ maxWidth: 1180, margin: '0 auto', padding: '34px 24px' }}>
        ...
        <div className="claimgrid">
          {t.claims.map((c) => (
            <div key={c} style={{ display: 'flex', alignItems: 'center', gap: 12, background: PANEL, border: `1px solid ${LINE}`, borderRadius: 14, padding: '15px 16px' }}>
              <span style={{ flexShrink: 0, width: 26, height: 26, borderRadius: 13, background: GREEN_TINT, color: ON_GREEN_TINT, fontSize: 13, fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✓</span>
        ...
        <section style={{ maxWidth: 1180, margin: '0 auto', padding: '0 24px 8px' }}>
          <div style={{ background: SAFFRON_TINT, borderRadius: 18, padding: '24px 26px', display: 'flex', gap: 18, alignItems: 'center', flexWrap: 'wrap', justifyContent: 'space-between' }}>
            <div style={{ maxWidth: 600 }}>
              <h3 style={{ fontSize: 20, fontWeight: 800, margin: '0 0 8px', color: ON_SAFFRON_TINT }}>CIS, sorted for {t.plural}.</h3>
              <p style={{ fontSize: 15, color: ON_SAFFRON_TINT, lineHeight: 1.6, margin: 0 }}>Working as a subcontractor? Text &ldquo;Dave paid £400, £80 CIS deducted&rdquo; and Lekhio logs the gross, keeps the deduction separate, and tracks it toward your refund. Most subbies are owed money back.</p>
            </div>
          </div>
        </section>
      ) : null}
      ...
      <section style={{ maxWidth: 1180, margin: '0 auto', padding: '34px 24px' }}>
        ...
        <div className="toolrow">
          {tools.map((tool) => (
            <Link key={tool.href} href={tool.href} style={{ background: PANEL, border: `1px solid ${LINE}`, borderRadius: 14, padding: '18px 16px', textAlign: 'center', color: INK }}>
      ...
      {/* CTA */}
      <section style={{ background: BAND }}>
        <div style={{ maxWidth: 680, margin: '0 auto', padding: '48px 24px', textAlign: 'center' }}>
          <h2 style={{ fontSize: 28, color: ON_BAND, fontWeight: 700, letterSpacing: '-0.6px', margin: '0 0 14px' }}>{Title}, your books are handled.</h2>
          <p style={{ fontSize: 16.5, color: ON_BAND_MUTED, lineHeight: 1.6, maxWidth: 520, margin: '0 auto 28px' }}>Snap it, say it, or text it. Lekhio logs it, claims it, and keeps you ready for tax. You always approve before anything reaches HMRC.</p>
          <Link href="/start" style={{ display: 'inline-block', backgroundColor: RIVER, color: ON_RIVER, fontSize: 16, fontWeight: 600, padding: '15px 32px', borderRadius: 12 }}>Start free trial</Link>
        </div>
      </section>
