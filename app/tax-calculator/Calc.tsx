...
// 🔴 ON a tint, the ink is the ON token. GREEN on GREEN_TINT reads 4.46:1 and SAFFRON_DEEP on
// SAFFRON_TINT reads 2.70:1. Both ON tokens are themed for dark too, so this is correct in both.
  ...
  return (
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 28, alignItems: 'start' }} className="calc-grid">
      ...
      <div style={{ background: 'var(--panel)', border: `1px solid ${LINE}`, borderRadius: 18, padding: 26, boxShadow: '0 16px 40px rgba(17,17,17,.06)' }}>
        ...
        <Line label="Total to set aside for tax" value={gbp(totalTax)} strong accent={ON_SAFFRON_TINT} />
        ...
        <div style={{ background: SAFFRON_TINT, borderRadius: 12, padding: '14px 16px', marginTop: 18 }}>
          <div style={{ fontSize: 13.5, fontWeight: 700, color: ON_SAFFRON_TINT, marginBottom: 4 }}>Set aside about {setAsidePct}p of every £1 of profit</div>
        ...
        {saved > 0 ? (
          <div style={{ background: GREEN_TINT, borderRadius: 12, padding: '14px 16px', marginTop: 12 }}>
            <div style={{ fontSize: 13.5, fontWeight: 700, color: ON_GREEN_TINT, marginBottom: 4 }}>Claiming your costs saved you about {gbp(saved)}</div>
