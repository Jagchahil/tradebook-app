...
export async function generateMetadata({ params }: { params: Promise<{ trade: string }> }): Promise<Metadata> {
  ...
  const description = `Lekhio is the WhatsApp back office for UK ${t.plural}. Snap receipts, log mileage and jobs, claim every cost, and stay MTD ready. ${t.cis ? 'CIS handled. ' : ''}7 days free, no card.`;
